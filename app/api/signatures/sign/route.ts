/**
 * Public - no session required. Each party uses their per-party token.
 * Body: {
 ...
 * honeypot + IP rate-limit. Bots can't submit a signature.
 */
import { NextRequest, NextResponse } from "next/server";
import { publicPartySign, publicPartyDecline } from "@/lib/data/signatures";
...
export async function POST(req: NextRequest) {
  ...
  if (body.decline) {
    const r = await publicPartyDecline(body.token, { reason: body.reason ?? "", ipAddress });
  ...
  const r = await publicPartySign(body.token, { signerName: body.signerName.trim(), ipAddress, userAgent });
  if ("error" in r) return NextResponse.json(r, { status: 400 });
  return NextResponse.json(r);
}
